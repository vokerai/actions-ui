    ...
    <DropdownMenu.Root preventScroll="{false}">
        <DropdownMenu.Trigger asChild let:builder>
            <Button
                ...
                variant="ghost"
                class="{cn(
                    'relative !right-12 h-6 w-fit items-center justify-center rounded-md bg-background/10 p-1 text-background/70 hover:bg-background/20 hover:text-background/80 dark:bg-foreground/5 dark:text-foreground/70 dark:hover:bg-foreground/10 dark:hover:text-foreground/80',
                    className,
                )}"
                {...$$restProps}
            >
