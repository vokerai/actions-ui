...
export function transformChunk(source: string, chunk: Chunk): string {
    ...
    const imports = lines
        ...
        .filter((line) =>
            chunk.dependencies.some((dep) => line.includes(` ${dep} `)),
        );
